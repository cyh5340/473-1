...
extern int schedule_me(float, int, int, int);
extern int num_preemeptions(int);
extern float total_wait_time(int);
...
void *worker_thread(void *arg)
{
	...
	scheduled_time = schedule_me(myinfo->arrival_time, myinfo->id, time_remaining, myinfo->priority);
	...
	while (time_remaining > 0)
	{
		...
		time_remaining -= 1.0;
		scheduled_time = schedule_me(get_global_time(), myinfo->id, time_remaining, myinfo->priority);
	}
	free(myinfo);
	pthread_exit(NULL);
}
...
{	//argc = 3;// ./out 0 input_1
	...
	printf("-------------------------------\n");
	printf("NumPreemptions  and TotalWaitingTime for all threads are:\n");
	tids = head_tids;
	while(tids)
	{
		printf("T%d: %d  %.1f\n", tids->id, num_preemeptions(tids->id), total_wait_time(tids->id));
		_tid_list_t* temp = tids;
		tids = tids->next;
		free(temp);
    }
